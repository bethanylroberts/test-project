...
function results = step3_validate_migration(options)
    ...
    try
        ...
        % Stage-1 baseline (surveys/rows extracted), if a split-summary log
        % from step1 is available. Written by SurveyFileWriter.finalize()
        % directly inside pending_dir; persists there even after step2 moves
        % individual survey files out, so this stays available across runs.
        try
            [baseline, baseline_file] = narwc.ingestion.load_split_summary(pending_dir);
            results.baseline = baseline;
            results.baseline.available = true;
            results.baseline_file = baseline_file;
        catch
            results.baseline = struct('available', false);
        end
        ...
        if options.DetailedErrorAnalysis && results.metrics.failed_uploads > 0
            fprintf('Analyzing errors (re-validating failed/pending surveys by rule)...\n');
            try
                batch_config = load_config(options.Batch);
            catch
                batch_config = load_config();
            end
            csv_files = collect_survey_csv_paths(failed_dir, pending_dir);
...
function metrics = calculate_migration_metrics(results, base_dir)
    ...
    % Stage-1 (extraction) baseline, if available, so the funnel covers
    % legacy-rows -> extracted-surveys -> attempted -> uploaded, not just
    % the post-step2 folder split.
...
function paths = collect_survey_csv_paths(varargin)
    % COLLECT_SURVEY_CSV_PATHS Full paths of survey CSVs (excluding files
    % starting with '_', e.g. the split-summary log) across one or more dirs.
    paths = {};
...
function display_validation_summary(results, failed_dir, log_dir) %#ok<INUSD>
    ...
    try
        ...
        % a step1 split-summary log is available.
        if isfield(m, 'extracted_surveys')
            fprintf('Stage Funnel:\n');
            fprintf('   Extracted (step 1):   %d surveys, %d rows\n', ...
                m.extracted_surveys, m.extracted_rows);
            fprintf('   Attempted (step 2):   %d surveys\n', m.total_records);
        ...
        if isfield(results, 'error_analysis')
            ...
            if ~isempty(ea.would_now_pass)
                fprintf('%d survey(s) currently in failed/ now re-validate CLEAN ', ...

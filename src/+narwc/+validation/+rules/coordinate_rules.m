...
function validate_latitude(data, collector, config)
    ...
    for i = 1:length(out_of_range)
        ...
        collector.addError('LAT_DD', row, ...
            sprintf('Latitude %.6g out of valid range [%.1f, %.1f]', ...
            lat(row), config.lat_min, config.lat_max), 'error', ...
    ...
    for i = 1:length(outside_survey)
        ...
        collector.addError('LAT_DD', row, ...
            sprintf('Latitude %.6g outside typical survey area [%.1f, %.1f]', ...
            lat(row), config.survey_lat_min, config.survey_lat_max), 'warning', ...
...
function validate_longitude(data, collector, config)
    ...
    for i = 1:length(out_of_range)
        ...
        collector.addError('LONG_DD', row, ...
            sprintf('Longitude %.6g out of valid range [%.1f, %.1f]', ...
            lon(row), config.lon_min, config.lon_max), 'error', ...
    ...
    for i = 1:length(outside_survey)
        ...
        eventno = get_eventno(data, row);
        collector.addError('LONG_DD', row, ...
            sprintf('Longitude %.6g outside typical survey area [%.1f, %.1f]', ...
            lon(row), config.survey_lon_min, config.survey_lon_max), 'warning', ...

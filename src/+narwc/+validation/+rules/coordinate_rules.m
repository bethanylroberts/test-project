...
function validate_latitude(data, collector, config)
    ...
    for i = 1:length(outside_survey)
        ...
        collector.addError('LAT_DD', row, ...
            sprintf('Latitude (%.1f) outside typical survey area [%.1f, %.1f]', ...
            data.LAT_DD(i), config.survey_lat_min, config.survey_lat_max), 'warning', ...
...
function validate_longitude(data, collector, config)
    ...
    for i = 1:length(outside_survey)
        ...
        collector.addError('LONG_DD', row, ...
            sprintf('Longitude (%.1f) outside typical survey area [%.1f, %.1f]', ...
            data.LONG_DD(i), config.survey_lon_min, config.survey_lon_max), 'warning', ...
